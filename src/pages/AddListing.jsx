...
export default function AddListing() {
  ...
  // Placeholder examples live in i18n under `ph.<key>`; t() returns the key
  // unchanged when it's missing, which is how we detect "no example for this
  // category/subcategory" and fall back to the generic hint.
  function phLookup(key) {
    if (!key) return null
    const titleKey = `ph.${key}.title`
    const title = t(titleKey)
    if (title === titleKey) return null
    return { title, desc: t(`ph.${key}.desc`) }
  }
  const placeholder =
    phLookup(form.subcategory) ||
    phLookup(form.category) ||
  ...
  async function handleSubmit() {
    ...
    // Upload images in parallel. If any upload fails, abort instead of
    // silently creating a listing with fewer photos than the user chose.
    ...
    if (uploaded.some(url => !url)) {
      setSubmitError(t('addListing.uploadError'))
      setSubmitting(false)
      return
    }
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6">
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            {form.category !== 'staff' && (
              <div>
                <label className="block text-sm font-medium text-navy mb-1.5">{t('addListing.keywordsLabel')}</label>
                <input
                  type="text"
                  value={form.keywords}
                  onChange={e => set('keywords', e.target.value)}
                  placeholder={t('addListing.keywordsPlaceholder')}
        ...
        {step === 4 && (
          <div>
            ...
            <div className="divide-y divide-gray-100">
              {[
                { label: t('addListing.reviewCategory'),    value: (() => { const c = categories.find(c => c.id === form.category); return c ? catalogLabel(c, 'cat') : '—' })() },
                ...(subcategories[form.category]?.length > 0
                  ? [{ label: t('addListing.reviewSubcategory'), value: (() => { const s = subcategories[form.category]?.find(s => s.id === form.subcategory); return s ? catalogLabel(s, 'subcat') : '—' })() }]
                  : []
                ),
      ...
      {feedbackStep && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 className="font-bold text-[#0A2342] mb-2">{t('addListing.feedbackTitle')}</h3>
            <p className="text-sm text-gray-500 mb-4">{t('addListing.feedbackDesc')}</p>
            ...
            <div className="flex gap-2 mb-4">
              ...
              >
                {t('addListing.feedbackSuggestion')}
              </button>
              <button
            ...
            <textarea
              ...
              placeholder={t('addListing.feedbackPlaceholder')}
              rows={3}
