...
export default function EditListing() {
  ...
  // Placeholder examples come from the shared i18n `ph.<key>` namespace;
  // t() returns the key unchanged when missing, which flags "no example".
  ...
  if (saved) {
    return (
      <div className="max-w-md mx-auto px-4 py-24 text-center">
        ...
        <h2 className="text-2xl font-bold text-navy mb-3">{t('editListing.updated')}</h2>
        <p className="text-gray-500">
          {editingAsAdmin ? t('editListing.adminRedirecting') : t('editListing.redirecting')}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      ...
      >
        <ChevronLeft size={16} /> {editingAsAdmin ? t('editListing.backToModeration') : t('listingDetail.back')}
      </Link>

      <h1 className={`text-2xl font-bold text-navy ${editingAsAdmin ? 'mb-2' : 'mb-8'}`}>{t('editListing.title')}</h1>
      ...
      {editingAsAdmin && (
        <div className="flex items-start gap-2.5 p-4 mb-6 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
          ...
          <span>{t('editListing.adminBanner')}</span>
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          ...
          {existingImages.length > 0 && (
            <div className="mb-4">
              <p className="text-xs text-gray-400 mb-2">{t('editListing.existingPhotos')}</p>
          ...
          {newPreviews.length > 0 && (
            <div className="mb-4">
              <p className="text-xs text-gray-400 mb-2">{t('editListing.newPhotos')}</p>
              <div className="grid grid-cols-4 gap-2">
          ...
          {totalImages < MAX_FILES ? (
            ...
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-xl text-xs text-gray-500">
              ...
              {t('editListing.maxPhotosReached')} ({MAX_FILES}/{MAX_FILES})
            </div>
          )}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
          ...
          <div>
            ...
            {!fieldErrors.price && form.price !== '' && Number(form.price) < 1 && (
              <p className="text-xs text-red-600 mt-1">{t('editListing.minPrice')}</p>
            )}
          </div>
